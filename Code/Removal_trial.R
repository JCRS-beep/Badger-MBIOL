                      ...
                      return.remvec = TRUE) 
 {
   ...
   for (i in 1:time) {   #  projection until end time
     ...
     if (as.character(i) %in% names(rem_index)) {  # if year i is present in remyear index
        ...
        goal <- round(Pop[remyear[1]] * (intensity/100))   # intended number to remove  - pop size beofre first rem
       ...
       if(rem_strat == "random"){
         ...
         # stage biased
       } else if(rem_strat != "random" && is.null(bias) == TRUE){   # is non random but no bias val provided...
         stop("please provide strength of bias for no random removal strategies")
         }  
       
       if(rem_strat != "random" && is.numeric(bias) == FALSE){   # for biased rems that are NOT index specific...
