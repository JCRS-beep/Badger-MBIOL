...
# setting up objs of future use
cnames <- c("Adults")  # for Dmat creation
# initial vector
...
# projection function -----
# meta.projection
meta.proj <- function(Umat, # matrix of stage-specific survival/growth transitions (will be the same in every patch)
  ...
  for (i in 1:time) {   # repeat for as many years as we have
    ...
    this_mat <- t(sapply(Vec, function(x) x[i,]))    # vec as a matrix, this years abundance per patch in each row
    movers_mat <- this_mat  
    movers_mat[,1] <- 0    # non moving individuals as 0 - following assumes only adults move
    movers_mat[,3] <- 0 
    
    move_mat <- matrix(0 , nrow = nrow(movers_mat), ncol = ncol(movers_mat))  # blank matrix to fill with individuals moving
    new_mat <- move_mat    # blank mat, fill with post movement abundances per patch
    for (p in 1:patches){    # all adults equal p moving? Adult f and m?
      # numbers moving out patch
        move_mat[p,] <- movers_mat[p,] * thisDmat[p,]    # only works if Dmat 1 row (movement equal across sexes). if not floor, still works?
      ...
      # moving INTO patch?
      new_vec <- this_mat[p,] - (movers_mat[p, ] * (thisDmat[p,])) + colSums(move_mat[-p,])   # those that remain in patch || vec - move out 
      # adding indivs that move in from other patches
      # if any negatives, set to 0
      
      new_mat[p, ] <- round(new_vec)
    }
...
params2 <- data.frame(Sc_max= rogers_cub_survival,   # max cub survival (equal for sexes), load from script rogers 1997
                     b=0.05,       # temp value ~0.05  
                     rep_K= rogers_k,          # max litter size (K), 
                     h= 6)

proj_test2 <- meta.proj(Umat,   # vector of stage names
...
# greater number of patches might make more realistic - how to set Ngroups?

# generating vec for 10 patches
n = 10
groups <- floor(runif(n, min = 3, max = 30))  # vector of group sizes per patch
initial_mat <- matrix(0, nrow = n, ncol = 4)
...
# more patches, stronger dd
rem_test2 <- meta.rem(Umat,   # vector of stage names
                     params2, # adjusted beta = 
                     stagenames = stages,
                     large_vec,   # list of initial abundances - string length patches * nStages
                     stagedist,  # proportion of each stage
...
# all go to extinction? too strong dd?
